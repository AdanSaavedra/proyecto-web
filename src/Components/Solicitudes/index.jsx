...
const Solicitudes = () => {
  ...
  const filteredHistorial = historial.filter(item => item.status === filter);
  ...
  return (
    <>
      <div className="buttons">
        <Butt prop="En espera" newclass={button3} click={opciones} value={3} setfil={() => setFilter('En espera')}/>
        <Butt prop="Aceptados" newclass={button4} click={opciones} value={4} setfil={() => setFilter('Aceptado')}/>
        <Butt prop="Rechazados" newclass={button5} click={opciones} value={5} setfil={() => setFilter('Rechazado')}/>
      </div>
      <div>
        <div id="contenedor-cards">
          {filteredHistorial.length === 0 && <p id="nohay">No hay solicitudes {filter}</p>}
